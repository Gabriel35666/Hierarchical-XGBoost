...
family_train_time = time.time() - t0
...
clastic_train_time = time.time() - t0
...
carbonate_train_time = time.time() - t0
# 总的训练时间
total_train_time = (
    family_train_time +
    clastic_train_time +
    carbonate_train_time
)
print("总的训练时间为:",total_train_time)

t0 = time.time()
# -----------预测融合------------
# 获取二分类（碳酸/碎屑）中为1的数据（碳酸）
family_prob = xgb_family.predict_proba(X_test)[:, 1]
family_pred = family_prob > 0.3  # 已验证：0.3为最优阈值
...
print("总的预测时间为:",total_pred_time)
# 单位样本预测时间
avg_pred_time_per_sample = total_pred_time / len(X_test)
print("单位样本预测时间为:",avg_pred_time_per_sample)
...
# df_predict.to_csv("./results/HXGBoost_井1预测结果.csv")
#
# -----------评估------------
...
#     "./results/HXGBoost_FeatureImportance_Well1.csv",
#     index=False
# )
# # 按照特征类型汇总贡献
# type_summary = (
...
# # 特征类型汇总贡献
# type_summary.to_csv(
#     "./results/HXGBoost_FeatureType_Contribution_Well1.csv",
#     index=False
# )
